...
## План доработок и статус

- **[План доработок](doc/PLAN_DORABOTKI.md)** — очереди задач (закрепление, админка, бэклог, опции, тесты).
- **[Прогресс / статус](PROGRESS.md)** — выполнение фаз P0–P64 и текущее состояние (в интерфейсе: футер → «Статус плана»; API: `GET /api/progress`).
