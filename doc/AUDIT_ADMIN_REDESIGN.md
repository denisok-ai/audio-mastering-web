...
### 1.3 Оставшиеся замечания по коду (актуализировано)

- **maintenance_mode:** ✅ реализовано — middleware в main.py, страница 503 «Сайт на обслуживании» для не-админских маршрутов.
- **feature_ai_enabled / feature_batch_enabled / feature_registration_enabled:** ✅ реализовано — в API: `require_feature_ai()` в ai_router, `require_feature_batch()` в mastering, `require_feature_registration()` в auth/register; флаги в /api/health; на фронте скрытие блоков по `_features` (app.js, login/register).
- **Журнал действий (AuditLog):** ✅ реализовано — раздел «Журнал» в админке, GET /api/admin/audit с фильтрами.
- **Валидация PATCH настроек:** ✅ добавлено — в admin.py для SettingsAppUpdate/SettingsLlmUpdate/SettingsSmtpUpdate заданы ограничения Pydantic Field (max_upload_mb 1–500, default_target_lufs −60…−1, port 1–65535, ai_limit_*, llm_guard_max_length_* и т.д.).
...
- **Дополнительные задачи** (бэклог):
  ...
  5. Опционально: **именованные шаблоны промптов** (template_name), кнопка «Применить шаблон» в редакторе промпта (см. [PLAN_DORABOTKI.md](PLAN_DORABOTKI.md) очередь 4).
  6. Опционально: отчёт **«Рекомендации по промптам»** на основе ai_usage_log и активных промптов с советами LLM (см. [PLAN_DORABOTKI.md](PLAN_DORABOTKI.md) очередь 4).
